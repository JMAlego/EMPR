...
void init_SEGMENTS(){
  ...
  I2C_SendBytes(EIGHT_SEG_ADDRESS, data, 6);
}
...
void SEGMENT_WriteHidden(int value, uint8_t dp_digit, int leading){
 ...
 I2C_SendBytes(EIGHT_SEG_ADDRESS, data, 5);
}
...
int main(){
  ...
  init_SEGMENTS();
  // value valid range -999 to 9999
  int value = 12.3;
  // zeros = 1, leading zeros shown
  // zeros = 0, no leading zeros shown
  int zeros = 0;

  SEGMENT_WriteFloat(value, zeros);
